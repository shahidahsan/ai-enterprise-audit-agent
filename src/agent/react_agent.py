...
def _run_tool_loop(llm_with_tools: BaseChatModel, tools_by_name: dict, messages: list[BaseMessage]) -> str:
...
def run_audit_query(
    ...
    final_text = _run_tool_loop(llm_with_tools, tools_by_name, messages)

    structurer = llm.with_structured_output(AuditFinding)
    return structurer.invoke(f"Format the following audit finding into the required structure:\n\n{final_text}")
